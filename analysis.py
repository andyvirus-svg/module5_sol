...
# Create a mapping from CustomerID to DateOfBirth
customer_dob = df_customers.set_index('CustomerID')['DateOfBirth']
# Map to orders_analysis
orders_analysis['DateOfBirth'] = orders_analysis['CustomerID'].map(customer_dob)
# Ensure datetime type
orders_analysis['DateOfBirth'] = pd.to_datetime(orders_analysis['DateOfBirth'], errors='coerce')
# Calculate age
orders_analysis['Age'] = ((reference_date - orders_analysis['DateOfBirth']) / np.timedelta64(1, 'Y')).astype(float)
...
print("\n✓ Chart saved as 'clustered_bar_chart.png'")
